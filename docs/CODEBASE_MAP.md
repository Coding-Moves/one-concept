...
| Authentication email | `backend/email-templates/` contains signup and recovery HTML with One Concept text branding; `docs/EMAIL_SETUP.md` covers manual replacement, the current Gmail SMTP choice, optional Resend setup, and real-inbox acceptance. These files are installed manually, not deployed by the app. |
