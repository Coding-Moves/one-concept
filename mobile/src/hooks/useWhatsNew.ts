...
import { CURRENT_VERSION, currentEntry, WhatsNewEntry } from '../data/whatsNew';
...
export function useWhatsNew(): WhatsNew {
  ...
  useEffect(() => {
    let active = true;
    (async () => {
      // No readable version, or no highlights defined for it: nothing to show.
      const found = currentEntry();
      if (!CURRENT_VERSION || !found) return;
  ...
  const dismiss = useCallback(() => {
    ...
    if (CURRENT_VERSION) setLastSeenVersion(CURRENT_VERSION);
  }, []);
