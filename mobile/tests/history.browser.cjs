...
(async()=>{
 ...
 try {
  ...
  await expect(page.getByPlaceholder('Search 50 loaded lessons')).toBeVisible();assert.equal(pageRequests,0);
  ...
  await expect(page.getByPlaceholder('Search 100 loaded lessons')).toBeVisible();
  await older('Load older lessons');
  await expect(page.getByPlaceholder('Search 120 loaded lessons')).toBeVisible();
  await page.getByRole('textbox',{name:'Search loaded history'}).fill('Historical lesson 119');
  ...
  await expect(page.getByPlaceholder('Search 50 loaded lessons')).toBeVisible();
  await older('Load older lessons');
  await expect(page.getByPlaceholder('Search 100 loaded lessons')).toBeVisible();
  await older('Load older lessons');
  await expect(page.getByPlaceholder('Search 120 loaded lessons')).toBeVisible();
