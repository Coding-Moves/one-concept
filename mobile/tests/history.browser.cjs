...
(async()=>{
 ...
 try {
  let online=true, failPage=false, holdPage=false, releasePage, pageRequests=0;
  ...
  await context.route('**/api/**',async route=>{
   ...
   else if(endpoint.startsWith('/v1/concepts/')) body={...concept,slug:decodeURIComponent(endpoint.split('/').pop()),title:'Historical lesson 119',summary:'Downloaded historical explanation.'};
  ...
  failPage=true;await page.getByRole('button',{name:'Load older lessons',exact:true}).click();
  await expect(page.getByRole('button',{name:'Retry older lessons',exact:true})).toBeVisible();failPage=false;
  await page.getByRole('button',{name:'Retry older lessons',exact:true}).click();
  await expect(page.getByText('100 of 120 learned concepts loaded.',{exact:true})).toBeVisible();
  await page.getByRole('button',{name:'Load older lessons',exact:true}).click();
  ...
  await page.getByRole('button',{name:'Load older lessons',exact:true}).click();await expect.poll(()=>!!releasePage).toBe(true);
