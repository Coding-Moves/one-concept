...
const { publicConfig, ApiConfigurationError } = await import('../src/api/config.ts');
const tick = () => new Promise(resolve => setImmediate(resolve));

test('missing API configuration neither fetches nor changes connectivity', async t => {
  const original = publicConfig.EXPO_PUBLIC_API_BASE_URL;
  publicConfig.EXPO_PUBLIC_API_BASE_URL = '';
  t.after(() => { publicConfig.EXPO_PUBLIC_API_BASE_URL = original; });
  t.mock.method(globalThis, 'fetch', () => { throw new Error('must not fetch'); });
  setConnectivity(true);
  await assert.rejects(apiRequest('/state'), ApiConfigurationError);
